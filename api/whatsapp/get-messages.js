export default async function handler(req, res) {
  ...
  const pickTextDeep = (m) => {
    ...
    const filtered = strs
      ...
      .filter((x) => !/^(true|false|null)$/i.test(x.s));
  ...
  try {
    ...
    // رتّب حسب الوقت ثم خذ آخر LIMIT (الأحدث)، وارجعهم بالترتيب الصحيح (قديم -> جديد)
    ...
    normalized.sort((a, b) => (a._t || 0) - (b._t || 0));
    const sliced = normalized.slice(Math.max(0, normalized.length - LIMIT)).map(({ _t, ...rest }) => rest);
