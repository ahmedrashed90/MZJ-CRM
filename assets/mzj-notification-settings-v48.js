...
function customerPhone(d){ return normalize(d.phoneNormalized || d.phone || d.mobile || d.phoneNumber || d.customerPhone || ""); }
...
function showToast({ type="newLead", title="إشعار", body="", sound="", iconUrl="" } = {}){
  ...
  item.addEventListener("click", () => { primeAudio(); playPreset(soundKey); });
  ...
  try{
    leadUnsub = onSnapshot(query(collection(db, "leads"), limit(1000)), snap => {
      ...
      if(added.length){
        ...
        showToast({ type:"newLead", title: currentSettings.newLeadTitle || "عميل جديد", body:`${customerName(l)}${customerPhone(l) ? " - " + customerPhone(l) : ""}` });
        pulseCashCard();
      }
    });
  ...
  try{
    msgUnsub = onSnapshot(query(collectionGroup(db, "messages"), limit(250)), snap => {
      ...
      snap.docChanges().forEach(ch => {
        ...
        if(msgIsIncoming(data)) fresh.push(data);
      });
      knownMsgIds = current;
      if(fresh.length){
        const m = fresh[fresh.length - 1];
        showToast({ type:"message", title: currentSettings.messageTitle || "رسالة جديدة من عميل", body: msgText(m).slice(0, 120) || "يوجد رسالة جديدة من عميل" });
      }
    });
...
onAuthStateChanged(auth, async user => {
  currentUser = user;
  await loadSettings();
  if(user){ startLeadNotifications(); startMessageNotifications(); }
});
