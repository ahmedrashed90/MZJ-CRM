...
(function(){
  ...
  const app = window.__MZJ_FIREBASE_APP__ || getApps()[0] || initializeApp(firebaseConfig);
  const auth = window.__MZJ_FIREBASE_AUTH__ || getAuth(app);
  const db = window.__MZJ_FIRESTORE_DB__ || getFirestore(app);
  ...
  function injectCss(){
    if(document.getElementById("mzj-sheet-import-v49-style")) return;
    const st=document.createElement("style"); st.id="mzj-sheet-import-v49-style"; st.textContent=`
