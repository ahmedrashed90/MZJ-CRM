...
(function(){
  ...
  function buildLead(row, idx, headers){
    ...
    const base = {
      ...
      updatedAt, lastUpdate:updatedAt, importedFromSheet:true, importSource:"admin_sheet_clean_import_v48"
    };
    const problems = [];
  ...
  async function waitForAuthUser(timeoutMs=8000){
    ...
    return await new Promise((resolve)=>{
      let done=false;
      const timer=setTimeout(()=>{ if(!done){ done=true; unsub(); resolve(auth.currentUser||null); } }, timeoutMs);
      const unsub=onAuthStateChanged(auth,(u)=>{ if(!done && u){ done=true; clearTimeout(timer); unsub(); resolve(u); } });
    });
  }
  async function saveRows(){
    if(!state.rows.length) return;
    const authUser = await waitForAuthUser();
    if(!authUser){ alert("لازم تسجل دخول قبل الاستيراد"); return; }
    ...
    try{
      for(let i=0;i<state.rows.length;i+=450){
        ...
        for(const r of part){
          const data = {...r.lead, importedAt: serverTimestamp(), importedByUid: authUser.uid, importedByEmail: authUser.email || "", updatedAt: r.lead.updatedAt || serverTimestamp()};
  ...
  function injectCss(){
    if(document.getElementById("mzj-sheet-import-v48-style")) return;
    const st=document.createElement("style"); st.id="mzj-sheet-import-v48-style"; st.textContent=`
